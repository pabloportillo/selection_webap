...
if (isset($_GET['success_duplicate'])) {
    ?>
<div class="alert alert-success">
      <a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
      Se ha duplicado la convocatoria correctamente.
</div>
    <?php
}

if (isset($_GET['fail_duplicate'])) {
    ?>
<div class="alert alert-danger">
      <a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
      La convocatoria no se pudo duplicar. Por favor, intentelo de nuevo.
</div>
    <?php
}
?>
...
<div class="convocatorias index large-9 medium-8 columns content">
					...
					<form>
						<div class="form-group row">
							<div class="col-xs-6">
								...
								<p style='color:grey'><font size="2" >Nombre de convocatoria</font></p>
							</div>
							<div class="col-xs-6">    
								<label for="estado">Estado</label>
								<select name="estado" class="form-control">
									<option value="Todos" <?php if(isset($_GET['estado']) && $_GET['estado'] =='Todos') echo 'selected="selected" ';?>>Todos</option>
									<option value="Visible" <?php if(isset($_GET['estado']) && $_GET['estado'] =='Visible') echo 'selected="selected" ';?>>Visible</option>
									<option value="Invisible" <?php if(isset($_GET['estado']) && $_GET['estado'] =='Invisible') echo 'selected="selected" ';?>>No Visible</option>
								</select>
							</div>
						</div>
    ...
    <table class="table table-striped table-bordered grid2" cellpadding="0" cellspacing="0" id='convocatorias_table'>
        ...
        <tbody>
            ...
            <tr style="cursor: pointer;" onclick="window.location='/convocatorias/view/<?=$convocatoria->id?>'">
