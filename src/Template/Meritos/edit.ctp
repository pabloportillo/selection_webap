...
<div class="meritos form large-9 medium-8 columns content">
    ...
    <fieldset>
        <?php $nombre = ($merito->tipos_meritos_id == 1) ? 'Formacion' : 'Experiencia' ?>
        <legend><?= __('Editar '.$nombre) ?></legend>
        <?php
	/*
        ...
        <div class="input select required" hidden>
           <label for="tipo_merito">Tipo de mérito</label>   
            <select name="tipo_merito" id="tipo_merito" class="form-control" required="required" >
                <option value="1" <?php if($merito->tipos_meritos_id == 1){ echo "selected";} ?>>No auto puntuable</option>
                <option value="2" <?php if($merito->tipos_meritos_id == 2){ echo "selected";} ?>>Auto puntuable</option>
            </select>
        </div>  
    </fieldset>  
